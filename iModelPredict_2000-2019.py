...
def main():
    data = pd.read_csv('Revenue2000_2019.csv')  # load data set
    ...
    ax1.set_xticks(np.arange(1999, 2021, 1))
    ax1.set_xticklabels(range(1999, 2021), rotation=45)
    ...
    while end <= 0:
        ...
        if Y1[-1] > (Y1[-2]*0.98):          # in local linear segment
            ...
            if k > 5:   # after year 5, adjust value with sigmoid 
